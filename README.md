...
- **Bead a Bracelet** (all ages) — Tap beads from the tray to thread them onto a string; each new bead drops into the biggest open spot on a growing loop (round, heart, star, flower, and diamond shapes). **Drag and drop a bead anywhere on the loop** — its neighbours gently push away to keep everything spaced (no overlaps). Tap a bead on the bracelet to take it off, or use ↩ Undo / ↺ Start over. **✨ Wear it!** clasps the loop into a golden circle with a sparkle celebration. A build-and-arrange interaction distinct from the tap-to-pop, fill, and read-along games.
- **Mermaid Bubbles** (all ages) — A little mermaid swims gently through an underwater scene with sand, seaweed, a starfish and a crab. **Drag the mermaid around** the water, or let her swim on her own. Bubbles rise from the seabed — **tap them to pop** (the mermaid also pops any she moves over). Coloured shells rest on the sand; **move the mermaid over a shell to collect it** (or tap it) into the tray at the top, which counts pink, purple, green, yellow and blue. The seabed stays anchored to the bottom in both portrait and landscape.
...
- **Story Time** (ages 4–6) — Five short decodable stories, shown one sentence per page. Under the words is a per-word progress bar: **tap a cell (or the word) to hear just that word**, then tap the next one to step through the sentence at your own pace. **▶ Again** reads the whole sentence; **Next** moves to the next sentence. Slow / Medium / Fast reading speed (default Fast).
