...
- **Bead a Bracelet** (all ages) — Tap beads from the tray to thread them onto a string; the beads arrange themselves evenly around a growing loop (round, heart, star, flower, and diamond shapes). **Drag a bead to slide it around the loop** to any spot you like (it moves just that bead — the row doesn't reorder). Tap a bead on the bracelet to take it off, or use ↩ Undo / ↺ Start over. **✨ Wear it!** clasps the loop into a golden circle with a sparkle celebration. A build-and-arrange interaction distinct from the tap-to-pop, fill, and read-along games.
